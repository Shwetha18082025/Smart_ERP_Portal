...
def profile(request):
    ...
    student__student__id=request.user.id, course__grade=request.user.grade
)
...
def profile_single(request, id):
    ...
    if request.GET.get("download_pdf"):
        if user.is_lecturer:
            ...
            courses = TakenCourse.objects.filter(
    student__student__id=id, course__grade=user.grade
)
...
def student_add_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # STEP 2 → Extract program from form.cleaned_data
            program = form.cleaned_data.get("program")   # Grade 1..10

            # STEP 3 → Set USER.GRADE (for attendance system)
            if program:
                grade_val = program.title.replace("Grade", "").strip()
                user.grade = grade_val
                user.save()

            # STEP 4 → Create Student model linked to User
            from .models import Student
            Student.objects.create(
                student=user,
                program=program,
                level=program.title  # optional
            )

            messages.success(
                request,
                f"Account for {first_name} {last_name} has been created. "
                f"An email with account credentials will be sent to {email}."
            )
