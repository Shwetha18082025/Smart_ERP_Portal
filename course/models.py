...
class Course(models.Model):
    ...
    # Keep program/level/semester fields removed or optional for school system
    # If you still need the original program FK for other parts of the app,
    # re-add it below, but for a school setup the above fields are sufficient.

    objects = CourseManager()

    def __str__(self):
        grade_part = f" - Grade {self.grade}" if self.grade else ""
        return "{0} ({1}){2}".format(self.title, self.code or "N/A", grade_part)
    ...
    def is_current_semester(self):
        # keep a safe fallback if core.Semester not present for school setup
        try:
